...
/*******sendto_file begin******/
#include <strings.h>
#define BUFFER_SIZE 1000             //发送文件udp缓冲区大小
...
/********sendto_file_end********/
    ...
    char *uxpath2 = CCNL_DEFAULT_UNIXSOCKNAME;
#else
    char *uxpath2 = NULL;
#endif
...
int sendto_file(struct ccnl_if_s *ifc, struct ccnl_buf_s *buf, sockunion *dest){
 ...
 // long int send_id = 0; 
  ...
  //int receive_id = 0; 
	...
	int file_fd = open("/home/istin/ccn_tmp_file", O_RDWR|O_CREAT|O_TRUNC,
			   S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
 	
	if(file_fd == -1)
	{
	DEBUGMSG(DEBUG, "open failed\n");
	return -1;
        }

	if (write(file_fd, buf->data, buf->datalen)!= buf->datalen) {
	DEBUGMSG(DEBUG, "write failed\n");
	return -1;
        }
	close(file_fd);
	DEBUGMSG(DEBUG, "write succeed\n");

	bzero((char *)&data,sizeof(data));

	 if(sendto(ifc->sock, (char*)&data, 0, 0, (struct sockaddr*) &dest->ux, sizeof(struct sockaddr_un)) < 0) 
      { 
	...
	return -1; 
      ...
      DEBUGMSG(DEBUG, "ccn send file to dtn end 0 char\n");
...
#ifdef USE_UNIXSOCKET
    case AF_UNIX:
      // if (buf->datalen >= 65508) {
        rc = sendto_file(ifc, buf, dest);
	DEBUGMSG(DEBUG, "unix sendto_file %s returned %d\n",
                 dest->ux.sun_path, rc);
     //   }
/*
        else {
                  ...
                  char *crypto_face_path)
{
    ...
    if (strcmp(uxpath2, CCNL_DEFAULT_UNIXSOCKNAME)!=0 && uxpath2!=NULL) {
        i = &relay->ifs[relay->ifcount];
        i->sock = ccnl_open_unixpath(uxpath2, &i->addr.ux);
        i->mtu = 4096;
        if (i->sock >= 0) {
            relay->ifcount++;
            DEBUGMSG(INFO, "UNIX interface 2 (%s) configured\n",
                     ccnl_addr2ascii(&i->addr));
            if (relay->defaultInterfaceScheduler)
                i->sched = relay->defaultInterfaceScheduler(relay,
                                                        ccnl_interface_CTS);
...
#endif
        case 'x':
        if(strcmp(uxpath, CCNL_DEFAULT_UNIXSOCKNAME)==0 || uxpath==NULL)
            uxpath = optarg;
        else
            uxpath2=optarg;
            break;
